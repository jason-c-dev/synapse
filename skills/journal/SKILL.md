...
3. **Find related notes** — call `vault_search` with key topics, names, and projects mentioned in the entry. Also check `vault_files` if needed.
...
5. **Check if today's journal exists** — call `vault_read` with `file: "YYYY-MM-DD Journal"`
...
6. **Link from daily note** — call `vault_daily_append`:
   - If this is a new journal: `\n- **Journal** — [[YYYY-MM-DD Journal|Journal entry]]: one-line summary`
   - If appending: no need to add another daily note link

## Vault Conventions
- Journal notes are named `YYYY-MM-DD Journal`
- Always include YAML frontmatter with `tags: [journal]` and `date`
